...
int low_level_fragmentation::ClasifyBox(const min_max_vectors& vects)
{
	...
	if (vects.first[0] == 0 && vects.second[0] == 0) return 3;  //Ãðàíèöà ïðÿìîóãîëüíèêà

	for (int i = 0; i < 4; i++)
	{
		if (vects.first[i] < 0) maxCount++; //Ïðîâåðêà âûïîëíåíèÿ óñëîâèÿ 4
		if (vects.second[i] > 0) return 0; //Ïðîâåðêà âûïîëíåíèÿ óñëîâèÿ 5
	}

	if (maxCount == 4) return 1;
	else return 2; //Ðàçáèâàåì íà 2 íîâûõ		
}
//------------------------------------------------------------------------------------------
void low_level_fragmentation::GetBoxType(const Box& box)
{
	...
	if (type == 0) //Óáèðàåì ïðÿìîóãîëüíèê èç ðàññìîòðåíèÿ
		not_solution->push_back(box);

	if (type == 1) //Ïðÿìîóãîëüíèê, â êîòîðîì åñòü ðåøåíèå (íóæíûé ýêñòðåìóì)
		solution->push_back(box);

	if (type == 2) { //Ïðÿìîóãîëüíèê, êîòîðûé íàäî ðàçáèòü
